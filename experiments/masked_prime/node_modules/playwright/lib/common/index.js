...
function dependenciesForTestFile(filename) {
  return fileDependencies.get(filename) || /* @__PURE__ */ new Set();
}
...
var FullProjectInternal = class {
  constructor(configDir, config, fullConfig, projectConfig, configCLIOverrides, packageJsonDir) {
    ...
    const configTrace = takeFirst(projectConfig.use?.trace, config.use?.trace);
...
var builtInReporters = ["list", "line", "dot", "json", "junit", "null", "github", "html", "blob", "perfetto"];
...
function calculateHash(content, filePath, isModule2, pluginsEpilogue) {
  const hash = import_crypto.default.createHash("sha1").update(isModule2 ? "esm" : "no_esm").update(content).update(filePath).update(version).update(pluginsEpilogue.map((p) => p[0]).join(",")).digest("hex");
  return hash;
}
...
function validateConfig(file2, config) {
  ...
  if ("retryStrategy" in config && config.retryStrategy !== void 0) {
    if (typeof config.retryStrategy !== "string" || !["immediate", "isolated"].includes(config.retryStrategy))
      throw errorWithFile(file2, `config.retryStrategy must be one of "immediate" or "isolated"`);
  }
