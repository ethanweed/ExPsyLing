...
async function gitDiff(gitDir, ci) {
  const diffLimit = 1e5;
  if (ci?.prBaseHash) {
    const hasBaseCommit = await runGit(["cat-file", "-e", `${ci.prBaseHash}^{commit}`], gitDir) !== void 0;
    if (!hasBaseCommit) {
...
var StripAnsiStream = class extends import_stream.Writable {
  ...
  write(chunk, encodingOrCallback, callback) {
    ...
    return this._target.write((0, import_util2.stripAnsiEscapes)(chunk.toString()), cb);
  }
};
var terminalScreen = (() => {
...
var TerminalReporter = class {
  ...
  generateSummaryMessage({ didNotRun: didNotRun2, skipped, expected, interrupted, unexpected, flaky, fatalErrors }) {
    const tokens = [];
    ...
    if (didNotRun2)
      tokens.push(this.screen.colors.yellow(`  ${didNotRun2} did not run`));
    if (expected)
...
var ProcessHost = class extends import_events.EventEmitter {
  ...
  async stop() {
    ...
    let lastHeartbeat = monotonicTime4();
    const onHeartbeat = () => lastHeartbeat = monotonicTime4();
    this.on("__heartbeat__", onHeartbeat);
    try {
      while (true) {
        const result = await raceAgainstDeadline2(() => exitPromise, lastHeartbeat + timeout);
        if (!result.timedOut)
          return;
        if (monotonicTime4() < lastHeartbeat + timeout)
          continue;
...
async function collectFilesForProject(project, fsCache = /* @__PURE__ */ new Map()) {
  ...
  const testMatch = (0, import_util5.createFileMatcher)(project.project.testMatch);
  const testIgnore = (0, import_util5.createFileMatcher)(project.project.testIgnore);
...
var ReporterTestRunImpl = class {
  constructor(_rootSuite, _readonlyProjectSuites) {
    ...
    this.skip = import_common3.transform.wrapFunctionWithLocation((location, target, reason) => this._modifier("skip", location, target, reason));
    this.fixme = import_common3.transform.wrapFunctionWithLocation((location, target, reason) => this._modifier("fixme", location, target, reason));
    this.fail = import_common3.transform.wrapFunctionWithLocation((location, target, reason) => this._modifier("fail", location, target, reason));
  }
  close() {
    this._active = false;
  }
  ...
  _checkTarget(method, target) {
    ...
    let suite = target instanceof import_common3.test.Suite ? target : target.parent;
    while (suite) {
      if (this._readonlyProjectSuites.has(suite)) {
        const targetType = target instanceof import_common3.test.TestCase ? " test" : "";
...
async function createRootSuite(testRun, errors, shouldFilterOnly) {
  ...
  if (config2.config.shard && !reporterTestRun.shouldSkipSharding()) {
    ...
    import_common4.suiteUtils.filterTestsRemoveEmptySuites(rootSuite, (test) => testsInThisShard.has(test));
  }
  if (testRun.postShardTestFilters.length)
    import_common4.suiteUtils.filterTestsRemoveEmptySuites(rootSuite, (test) => testRun.postShardTestFilters.every((filter) => filter(test)));
  ...
  rootSuite._entries.sort((a, b) => config2.projects.indexOf(a._fullProject) - config2.projects.indexOf(b._fullProject));
  testRun.rootSuite = rootSuite;
  testRun.topLevelProjects = topLevelProjects;
}
function createProjectSuite(project, fileSuites) {
  const projectSuite = new import_common4.test.Suite(project.project.name, "project");
  for (const fileSuite of fileSuites)
    projectSuite._addSuite(import_common4.suiteUtils.bindFileSuiteToProject(project, fileSuite));
  const grepMatcher = (0, import_util6.createTitleMatcher)(project.project.grep);
  const grepInvertMatcher = project.project.grepInvert ? (0, import_util6.createTitleMatcher)(project.project.grepInvert) : null;
  import_common4.suiteUtils.filterTestsRemoveEmptySuites(projectSuite, (test) => {
...
function filterProjectSuite(projectSuite, testFilters) {
  ...
  import_common4.suiteUtils.filterTestsRemoveEmptySuites(result, (test) => testFilters.every((filter) => filter(test)));
  return result;
}
function buildProjectSuite(project, projectSuite) {
  ...
  for (const fileSuite of projectSuite.suites) {
    ...
    for (let repeatEachIndex = 1; repeatEachIndex < project.project.repeatEach; repeatEachIndex++) {
      ...
      import_common4.suiteUtils.applyRepeatEachIndex(project, clone, repeatEachIndex);
...
async function requireOrImportDefaultFunction(file, expectConstructor) {
  ...
  if (typeof func !== "function")
    throw (0, import_util6.errorWithFile)(file, `file must export a single ${expectConstructor ? "class" : "function"}.`);
  return func;
}
...
async function loadTestList(config2, filePath) {
  try {
    ...
    const descriptions = lines.map((line) => {
      ...
      return { project, file: toPosixPath((0, import_util6.parseLocationArg)(tokens[0]).file), titlePath: tokens.slice(1) };
    });
    ...
    throw (0, import_util6.errorWithFile)(filePath, "Cannot read test list file: " + e.message);
  }
}
...
var GitHubLogger = class {
  ...
  _log(message, type = "notice", options = {}) {
    ...
    process.stdout.write((0, import_util7.stripAnsiEscapes)(`::${type} ${configs}::${message}
`));
  }
...
`);
  ...
  _resolveOptions() {
    const outputFolder = reportFolderFromEnv() ?? (0, import_util8.resolveReporterOutputPath)("playwright-report", this._options.configDir, this._options.outputFolder);
    return {
      outputFolder,
...
function standaloneDefaultFolder() {
  return reportFolderFromEnv() ?? (0, import_util8.resolveReporterOutputPath)("playwright-report", process.cwd(), void 0);
}
...
function createErrorCodeframe(message, location) {
  ...
  return babel2.codeFrameColumns(
    ...
    },
    {
      ...
      linesAbove: 100,
      linesBelow: 100,
      message: (0, import_util8.stripAnsiEscapes)(message).split("\n")[0] || void 0
    }
  );
}
...
var JSONReporter = class {
  ...
  _mergeSuites(suites) {
    ...
    for (const projectSuite of suites) {
      const projectId = import_common5.config.getProjectId(projectSuite.project());
      const projectName = projectSuite.project().name;
      for (const fileSuite of projectSuite.suites) {
...
Warning: attachment ${attachmentPath} is missing`);
  ...
  async _buildRetryEntry(result, prefix) {
    ...
    entry.children.push({ name: "stackTrace", text: (0, import_util9.stripAnsiEscapes)(stackTrace) });
...
function classifyResultError(result) {
  ...
  if (!error)
    return null;
  const rawMessage = (0, import_util9.stripAnsiEscapes)(error.message || error.value || "");
...
var ListReporter = class extends TerminalReporter {
  ...
  _testPrefix(index, statusMark) {
    const statusMarkLength = (0, import_util10.stripAnsiEscapes)(statusMark).length;
...
// packages/playwright/src/reporters/perfetto.ts
var import_fs8 = __toESM(require("fs"));
var import_path12 = __toESM(require("path"));
var import_zlib = __toESM(require("zlib"));
var import_coreBundle2 = require("playwright-core/lib/coreBundle");
var import_util11 = require("../util");
var { toPosixPath: toPosixPath4 } = require("playwright-core/lib/coreBundle").utils;
var kStatusColors = {
  passed: "good",
  failed: "bad",
  timedOut: "terrible",
...
var kProcessId = 1;
var kRunThreadId = 0;
var PerfettoReporter = class {
  constructor(options) {
    this._events = [];
    this._laneEndTime = /* @__PURE__ */ new Map();
    this._globalErrors = [];
  ...
  _allocateLane(preferredLane, startMs) {
  ...
  _appendStep(step, tid, parentStart, parentEnd) {
  ...
  _formatError(error) {
    return (0, import_util11.stripAnsiEscapes)(formatError(nonTerminalScreen, error).message);
  }
  _relativePath(file) {
    return toPosixPath4(import_path12.default.relative(this._config.rootDir, file));
  }
  async _writeReport(result) {
    let timeOrigin2 = result.startTime.getTime();
    for (const event of this._events)
      timeOrigin2 = Math.min(timeOrigin2, event.ts);
    ...
    for (const event of this._events) {
      event.ts = Math.round((event.ts - timeOrigin2) * 1e3);
      if (event.dur !== void 0)
        event.dur = Math.round(event.dur * 1e3);
    }
    const metadata = {
      "playwright-version": (0, import_coreBundle2.getPlaywrightVersion)(),
      "start-time": result.startTime.toISOString(),
      "duration": result.duration,
      "status": result.status
    };
    await import_fs8.default.promises.mkdir(import_path12.default.dirname(this._resolvedOutputFile), { recursive: true });
    ...
    for (const events of [metadataEvents, this._events]) {
      for (const event of events) {
        await writer.write(separator2 + JSON.stringify(event));
        separator2 = ",";
      }
    }
...
var ChunkWriter = class {
  constructor(file) {
    const fileStream = import_fs8.default.createWriteStream(file);
    const gzip = file.endsWith(".gz") ? import_zlib.default.createGzip() : void 0;
    gzip?.pipe(fileStream);
    this._stream = gzip ?? fileStream;
    this._closed = new Promise((resolve) => fileStream.on("close", resolve));
    for (const stream of /* @__PURE__ */ new Set([this._stream, fileStream]))
...
var perfetto_default = PerfettoReporter;
...
function computeCommandHash(config2, runOptions) {
  ...
  if (runOptions?.testList)
    command.testList = calculateSha13(import_fs9.default.readFileSync(runOptions.testList));
  if (runOptions?.testListInvert)
    command.testListInvert = calculateSha13(import_fs9.default.readFileSync(runOptions.testListInvert));
...
async function applySuggestedRebaselines(config2, reporter, filteredProjects) {
  ...
  const patchFile = import_path13.default.join(project.project.outputDir, "rebaselines.patch");
  for (const fileName of [...suggestedRebaselines.keys()].sort()) {
    const source = await import_fs10.default.promises.readFile(fileName, "utf8");
    const lines = source.split("\n");
    ...
    const relativeName = import_path13.default.relative(process.cwd(), fileName);
    files.push(relativeName);
    if (config2.config.updateSourceMethod === "overwrite") {
      await import_fs10.default.promises.writeFile(fileName, result);
    } else if (config2.config.updateSourceMethod === "3way") {
      await import_fs10.default.promises.writeFile(fileName, applyPatchWithConflictMarkers(source, result));
    } else {
      const gitFolder = findGitRoot(import_path13.default.dirname(fileName), gitCache);
      const relativeToGit = import_path13.default.relative(gitFolder || process.cwd(), fileName);
...
`);
  if (config2.config.updateSourceMethod === "patch") {
    await import_fs10.default.promises.mkdir(import_path13.default.dirname(patchFile), { recursive: true });
    await import_fs10.default.promises.writeFile(patchFile, patches.join("\n"));
    reporter.onStdErr(`
  ` + colors3.cyan("git apply " + import_path13.default.relative(process.cwd(), patchFile)) + "\n");
  }
}
...
function findGitRoot(dir, cache) {
  ...
  if (result !== void 0)
    return result;
  const gitPath = import_path13.default.join(dir, ".git");
  if (import_fs10.default.existsSync(gitPath) && import_fs10.default.lstatSync(gitPath).isDirectory()) {
    cache.set(dir, dir);
    return dir;
  }
  const parentDir = import_path13.default.dirname(dir);
  if (dir === parentDir) {
    cache.set(dir, null);
    return null;
  }
...
var WorkerHost = class extends ProcessHost {
  constructor(testGroup, options) {
    ...
    this._params = {
      ...
      artifactsDir: import_path14.default.join(options.outputDir, artifactsFolderName(workerIndex)),
      pauseOnError: options.pauseOnError,
      pauseAtEnd: options.pauseAtEnd
    };
  }
  async start() {
    await import_fs11.default.promises.mkdir(this._params.artifactsDir, { recursive: true });
    return await this.startRunner(this._params, {
      onStdOut: (chunk) => this.emit("stdOut", import_common6.ipc.stdioChunkToParams(chunk)),
      onStdErr: (chunk) => this.emit("stdErr", import_common6.ipc.stdioChunkToParams(chunk))
    });
  }
  async onExit() {
...
var Dispatcher = class {
  ...
  async _runJobInWorker(index, jobDispatcher) {
    ...
    let startError;
    if (!worker) {
      worker = this._createWorker(job, index, import_common7.ipc.serializeConfig(this._testRun.config, true));
...
var JobDispatcher = class {
  ...
  _onDone(params) {
    ...
    const remainingJob = remaining.length ? { ...this.job, tests: remaining } : void 0;
    const isolatedRetriesJob = isolatedRetries.length ? { ...this.job, tests: isolatedRetries } : void 0;
  ...
  _onTestPaused(worker, params) {
    ...
    const sendMessage = async (message) => {
      try {
        ...
        const error = (0, import_util12.serializeError)(e);
    ...
    const taskLoop = async () => {
      for (const task of this._tasks) {
        ...
        try {
          ...
          errors.push((0, import_util13.serializeError)(e));
        } finally {
...
async function detectChangedTestFiles(baseCommit, configDir) {
  ...
  const untrackedFiles = gitFileList(["ls-files", "--others", "--exclude-standard"]).map((file) => import_path15.default.join(configDir, file));
  const [gitRoot] = gitFileList(["rev-parse", "--show-toplevel"]);
  const trackedFilesWithChanges = gitFileList(["diff", baseCommit, "--name-only"]).map((file) => import_path15.default.join(gitRoot, file));
  return new Set(import_common8.cc.affectedTestFiles([...untrackedFiles, ...trackedFilesWithChanges]));
}
...
var readDirAsync2 = (0, import_util14.promisify)(import_fs12.default.readdir);
var TestRun = class {
...
function createClearCacheTask(config2) {
  return {
    title: "clear cache",
    setup: async () => {
      await (0, import_util15.removeDirAndLogToConsole)(import_common9.cc.cacheDir);
    }
  };
}
    ...
    setup: async (testRun) => {
      ...
      await Promise.all(Array.from(outputDirs).map((outputDir) => removeFolders4([outputDir]).then(async ([error]) => {
        ...
        if (error.code === "EBUSY") {
          ...
          await Promise.all(entries.map((entry) => removeFolders4([import_path16.default.join(outputDir, entry)])));
        } else {
          throw error;
        }
      })));
...
function createListFilesTask() {
  return {
    ...
    setup: async (testRun, errors) => {
      ...
      for (const [project, files] of testRun.projectFiles) {
        const projectSuite = new import_common9.test.Suite(project.project.name, "project");
        projectSuite._fullProject = project;
        testRun.rootSuite._addSuite(projectSuite);
        const suites = files.map((file) => {
          const title = import_path16.default.relative(testRun.config.config.rootDir, file);
  ...
  return {
    ...
    setup: async (testRun, errors, softErrors) => {
      if (testRun.options.locations?.length) {
        const { testFilter, fileFilter } = import_common9.suiteUtils.createFiltersFromArguments(testRun.options.locations);
      ...
      if (testRun.options.grep || testRun.options.grepInvert) {
        const grepMatcher = testRun.options.grep ? (0, import_util15.createTitleMatcher)((0, import_util15.forceRegExp)(testRun.options.grep)) : () => true;
        const grepInvertMatcher = testRun.options.grepInvert ? (0, import_util15.createTitleMatcher)((0, import_util15.forceRegExp)(testRun.options.grepInvert)) : () => false;
    ...
    setup: async (testRun) => {
      ...
      for (let i = 0; i < projectToSuite.size; i++) {
        ...
        for (const ignoreMaxFailures of [false, true]) {
          ...
          const phase = { dispatcher: new Dispatcher(testRun, { ignoreMaxFailures }), projects: [] };
...
var LastRunReporter = class {
  constructor(filteredProjects, listMode, lastFailedFileOverride) {
    ...
    if (override) {
      this._lastRunFile = import_path17.default.resolve(process.cwd(), override);
    } else {
      const [project] = filteredProjects;
      if (project)
        this._lastRunFile = import_path17.default.join(project.project.outputDir, ".last-run.json");
    }
  }
  async filterLastFailed() {
    if (!this._lastRunFile)
      return void 0;
    try {
      const lastRunInfo = JSON.parse(await import_fs13.default.promises.readFile(this._lastRunFile, "utf8"));
  ...
  async onEnd(result) {
    ...
    const lastRunInfo = {
      status: result.status,
      failedTests: this._suite?.allTests().filter((t2) => !t2.ok() || didNotRun(t2)).map((t2) => t2.id) || []
    };
    await import_fs13.default.promises.mkdir(import_path17.default.dirname(this._lastRunFile), { recursive: true });
    await import_fs13.default.promises.writeFile(this._lastRunFile, JSON.stringify(lastRunInfo, void 0, 2));
  }
};
...
var TestRunner = class extends import_events2.default {
  constructor(configLocation, configCLIOverrides) {
    ...
    this._watcher = new FSWatcher((events) => {
      ...
      events.forEach((f) => import_common10.cc.collectAffectedTestFiles(f.file, collector));
  ...
  hasSomeBrowsers() {
    for (const browserName of ["chromium", "webkit", "firefox"]) {
      try {
        import_coreBundle3.registry.registry.findExecutable(browserName).executablePathOrDie("javascript");
        return true;
      } catch {
      }
    }
  ...
  async installBrowsers() {
    const executables = import_coreBundle3.registry.registry.defaultExecutables();
    await import_coreBundle3.registry.registry.install(executables);
  }
  async loadConfig() {
  ...
  async watch(fileNames) {
    ...
    for (const fileName of fileNames) {
      ...
      import_common10.cc.dependenciesForTestFile(fileName).forEach((file) => this._watchedTestDependencies.add(file));
    }
  ...
  async _loadConfig(overrides) {
    try {
      const config2 = await import_common10.configLoader.loadConfig(this.configLocation, overrides);
      if (!this._plugins) {
      ...
      return { config: null, error: (0, import_util16.serializeError)(e) };
    }
  }
  lastLoadedConfig() {
...
var import_util17 = __toESM(require("util"));
...
var TestServer = class {
  ...
  async start(options) {
    ...
    return await import_coreBundle4.server.startTraceViewerServer({
      host: options.host,
      port: options.port,
  ...
  _setInterceptStdio(interceptStdio) {
    ...
    if (interceptStdio) {
      if (debug6.log === originalDebugLog) {
        debug6.log = (...args) => {
          const string = import_util17.default.format(...args) + "\n";
...
async function runUIMode(configFile, configCLIOverrides, options) {
  const configLocation = import_common11.configLoader.resolveConfigLocation(configFile);
  return await innerRunTestServer(configLocation, configCLIOverrides, options, async (server, cancelPromise) => {
    await import_coreBundle4.server.installRootRedirect(server, void 0, { ...options, webApp: "uiMode.html" });
    if (options.host !== void 0 || options.port !== void 0) {
      await import_coreBundle4.server.openTraceInBrowser(server.urlPrefix("human-readable"));
    } else {
      const channel = await installedChromiumChannelForUI(configLocation, configCLIOverrides);
      const page = await import_coreBundle4.server.openTraceViewerApp(server.urlPrefix("precise"), "chromium", {
...
async function installedChromiumChannelForUI(configLocation, configCLIOverrides) {
  const config2 = await import_common11.configLoader.loadConfig(configLocation, configCLIOverrides).catch((e) => null);
  if (!config2)
    return void 0;
...
var import_coreBundle5 = require("playwright-core/lib/coreBundle");
...
async function toggleShowBrowser() {
  if (!showBrowserServer) {
    showBrowserServer = new import_coreBundle5.remote.PlaywrightServer({ mode: "extension", path: "/" + createGuid3(), maxConnections: 1 });
...
async function extractAndParseReports(dir, shardFiles, internalizer, printStatus) {
  ...
  for (const file of shardFiles) {
    ...
    printStatus(`extracting: ${(0, import_util18.relativeFilePath)(absolutePath)}`);
