...
const { getActionGroup, renderParamsForCall, renderSubtitleForCall, renderTitleForCall, truncateParam } = require("playwright-core/lib/coreBundle").iso;
    ...
    const csiListener = {
      onApiCallBegin: (data, channel) => {
        ...
        if (channel.type === "Tracing" && channel.method === "tracingGroupEnd") {
          data.userData = (error) => tracingGroupSteps.pop()?.complete({ error });
          return;
        }
        ...
        if (channel.type === "Tracing" && channel.method === "tracingGroup") {
          ...
          data.userData = (error) => step.complete({ error });
        }
      },
      onApiCallEnd: (data) => {
        const completeStep = data.userData;
...
const playwrightFixtures = {
  ...
  contrast: [({ contextOptions }, use) => use(contextOptions.contrast === void 0 ? "no-preference" : contextOptions.contrast), { option: true, box: true }],
  ...
  forcedColors: [({ contextOptions }, use) => use(contextOptions.forcedColors === void 0 ? "none" : contextOptions.forcedColors), { option: true, box: true }],
  ...
  reducedMotion: [({ contextOptions }, use) => use(contextOptions.reducedMotion === void 0 ? "no-preference" : contextOptions.reducedMotion), { option: true, box: true }],
...
class ArtifactsRecorder {
  ...
  async didFinishTest() {
    ...
    await Promise.all(leftoverContexts.map(async (context2) => {
      await this._stopTracing(context2.request, context2.request.tracing);
...
function renderSubtitle(type, method, params) {
  const subtitle = renderSubtitleForCall({ type, method, params });
  return subtitle === void 0 ? void 0 : truncateParam(subtitle);
}
function tracing() {
