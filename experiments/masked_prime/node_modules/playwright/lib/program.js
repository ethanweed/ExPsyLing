...
function addInitSkillsCommand(program2) {
  ...
  command.action(async (opts) => {
    try {
      await import_coreBundle.tools.installSkills(import_coreBundle.tools.allSkills, opts.loop);
    } catch (e) {
      console.error(e);
      gracefullyProcessExitDoNotHang(1);
    }
...
const testOptions = [
  ["--add-reporter <reporter>", { description: `Reporter to add on top of the configured reporters, comma-separated, can be ${import_common.builtInReporters.map((name) => `"${name}"`).join(", ")} or a path to a reporter module` }],
  /* deprecated */
