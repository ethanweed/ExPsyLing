...
for i in *.md ; do echo "$i" && pandoc metadata.yaml -s -t revealjs -s -V revealjs-url=https://unpkg.com/reveal.js/  --include-in-header=styles.css $i --filter=/Users/ethan/opt/miniconda3/lib/python3.9/site-packages/pandoc_mermaid_filter.py -o html/"${i%.*}".html; done
