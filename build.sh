...
for i in *.md ; do echo "$i" && pandoc metadata.yaml -s -t revealjs --mathjax -s -V revealjs-url=https://unpkg.com/reveal.js/  --include-in-header=styles.css $i --filter mermaid-filter  -o html/"${i%.*}".html; done
